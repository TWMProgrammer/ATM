...
export class GitService {
    private fileBlameCache = new Map<string, Map<number, BlameLineInfo>>();
    ...
    private async _executeBlame(filepath: string): Promise<void> {
        try {
            ...
            const { stdout } = await execAsync(`git blame --line-porcelain "${filename}"`, { cwd, maxBuffer: 10 * 1024 * 1024 });
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (currentCommitHash && !this.commitCache.has(currentCommitHash)) {
                if (line.startsWith('author ')) {
                    ...
                    if (currentCommitInfo.hash && currentCommitInfo.authorName && currentCommitInfo.authorEmail && currentCommitInfo.date && currentCommitInfo.summary) {
    ...
    public async getLineDiff(filepath: string, commitHash: string, originalLine: number): Promise<LineDiffInfo | null> {
        if (commitHash === '0000000000000000000000000000000000000000') { return null; }
        try {
            const cwd = path.dirname(filepath);
            const filename = path.basename(filepath);
            const { stdout } = await execAsync(`git show ${commitHash} --no-color -U3 -- "${filename}"`, { cwd, maxBuffer: 5 * 1024 * 1024 });
    ...
    private extractLineDiffFromShow(showOutput: string, targetLine: number): LineDiffInfo | null {
        ...
        for (const line of lines) {
          ...
          if (line.startsWith('-')) {
            ...
            if (currentNewLine === targetLine) { current = line.substring(1); }
            currentNewLine++;
          } else {
            currentNewLine++;
          }

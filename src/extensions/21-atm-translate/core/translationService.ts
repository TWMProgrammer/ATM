...
import {
	getSpellcheckProviderOrder,
	getTranslationProviderOrder,
	providerNames,
	readConfiguration,
	translationSecretKeys,
} from './config';
...
export { translationSecretKeys };
export type { SpellcheckRequest, TranslationProviderId, TranslationRequest, TranslationResult };
...
export async function correctPlainText(
	...
	if (cleanText.length > maxTextLength) {
		throw new Error(`Text is too long. Keep spelling checks under ${maxTextLength} characters.`);
	}
